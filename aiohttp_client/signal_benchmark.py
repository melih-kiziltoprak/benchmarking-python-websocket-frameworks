...
MSG_CNT = 333
MSG_CNT_VALUES = [1, 10, 50, 100, 500, 1000, 2000]
MAX_CONN = 602
STEP_COUNT = 10
        ...
        async with session.ws_connect("ws://localhost:5001/ws") as websocket:
            ...
            for i in range(message_count):
                ...
                if message.startswith("0"):
                    message_value = float(message)
                    # BUY signal with 20% probability:
                    if message_value < 0.2:
                        await websocket.send_str(f"BUY {id} - {i}")
                    # SELL signal with 20% probability:
                    elif message_value > 0.8:
                        await websocket.send_str(f"SELL {id} - {i}")
                elif message.startswith("O"):
                    print(f"Received acknowledgment id:{id} - iteration:{i}: '{message}'")
                else:
                    print(f"FAIL {message}\n")

            latency = time.time() - start_time
            print(f"id#{id}: {message_count} messages received in {latency} seconds.")
...
async def main():
    for msg_cnt in MSG_CNT_VALUES:
        ...
        output_directory = f'{SERVER}_signal_output_graphs'
        ...
        csv_filename = os.path.join(f'signal_benchmark.csv')
